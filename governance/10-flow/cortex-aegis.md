...
**Cortex-Aegis** is the automated assurance system that validates agentic development workflows against governance, security, and quality requirements. It serves as the "guardrail" for AI agents and humans, ensuring that plans, implementations, and changes align with Cortex-OS principles and compliance requirements.
...
- **Feature Implementation (G2 + G5)**: All feature flows must call Aegis at plan validation (G2) and verification (G5) stages
...
- [Unified Checklists](../20-checklists/checklists.cortex-os.md) - Section 7 Aegis requirements
...
4. Map each finding or mitigation to the applicable OWASP LLM Top 10 control ID(s) listed in `/.cortex/rules/llm-threat-controls.md`; include the mapping in the PR evidence comment.
