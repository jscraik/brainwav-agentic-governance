...
const { govRoot, indexPath, agentsPath, pointerPath, packageRoot, mode, pointer } =
 ...
 */
const workflowBase = path.relative(
	repoRoot,
	path.join(govRoot, '10-flow', 'agentic-coding-workflow.md')
);

const workflows = {
	feature: `${workflowBase}#51-feature-implementation-flow`,
	fix: `${workflowBase}#53-fix-implementation-flow`,
	refactor: `${workflowBase}#54-refactor--cleanup-flow`,
	research: `${workflowBase}#52-research--spike-flow`,
	code_review: `${workflowBase}#55-code-review-flow`
};
...
function main() {
	...
	const governanceIndex = JSON.parse(fs.readFileSync(indexPath, 'utf8'));

	const context = {
		repo_root: repoRoot,
		agents_md_path: agentsPath,
		agents_md_sha: agentsSha,
		governance_index_path: indexPath,
